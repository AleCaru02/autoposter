...
import { decryptTokenBundle, linkedinVersion, processDuePublications } from "../api/_lib/social.js";
...
async function fixture(sql, marker, profileId) {
  ...
  for(const capability of ["schedule.job.create","social.publish.scheduled","social.facebook.publish","social.instagram.publish","social.linkedin.publish","analytics.read"]){
...
async function facebookPreflight(sql, marker, profileId, env) {
  ...
  const candidates=(Array.isArray(body.data)?body.data:[]).filter((item)=>typeof item?.id==="string"); const headers={authorization:`Bearer ${row.accessToken}`,accept:"application/json"}; const diagnostics={shares:[],post_impressions:[]};
  // Select an existing post only when the provider returns at least one real
  // numeric metric. The product validates the object with `id` and fetches each
  // optional counter independently, so the verifier must mirror that contract
  // instead of requiring Meta's rejected all-fields combination.
  for(const post of candidates){const probe=new URL(`https://graph.facebook.com/${version}/${encodeURIComponent(post.id)}`);probe.searchParams.set("fields","shares");const checked=await fetch(probe,{headers});const payload=await checked.json().catch(()=>({}));const count=Number(payload?.shares?.count);diagnostics.shares.push(checked.status);if(checked.ok&&Number.isFinite(count))return {ready:true,remotePostId:post.id,source:"PAGE_PUBLISHED_POSTS_SHARES_PROBE",pagination:Boolean(body.paging?.next),candidatesChecked:diagnostics.shares.length};}
  for(const post of candidates){const probe=new URL(`https://graph.facebook.com/${version}/${encodeURIComponent(post.id)}/insights`);probe.searchParams.set("metric","post_impressions");const checked=await fetch(probe,{headers});const payload=await checked.json().catch(()=>({}));const value=Number(payload?.data?.[0]?.values?.[0]?.value);diagnostics.post_impressions.push(checked.status);if(checked.ok&&Number.isFinite(value))return {ready:true,remotePostId:post.id,source:"PAGE_PUBLISHED_POSTS_IMPRESSIONS_PROBE",pagination:Boolean(body.paging?.next),candidatesChecked:candidates.length+diagnostics.post_impressions.length};}
  return {ready:false,status:diagnostics.post_impressions.at(-1)||diagnostics.shares.at(-1)||404,error:"FACEBOOK_NO_EXISTING_POST_WITH_REAL_METRICS",candidatesChecked:candidates.length,diagnostics};
}
async function instagramPreflight(sql, marker, profileId, env) {
  if(!await ownedProfile(sql,marker,profileId)) throw new Error("QA_PROFILE_SCOPE_MISMATCH"); const row=await connection(sql,profileId,"INSTAGRAM",env); const granted=Array.isArray(row.permissions)?row.permissions:[];
  if(!granted.includes("instagram_manage_insights"))return {ready:false,status:403,analyticsBlocked:true,error:"MISSING_instagram_manage_insights",source:"LOCAL_SCOPE_GATE"};
  const version=/^v\d+\.\d+$/.test(env.META_GRAPH_VERSION||"")?env.META_GRAPH_VERSION:"v23.0"; const headers={authorization:`Bearer ${row.accessToken}`,accept:"application/json"};
  const mediaUrl=new URL(`https://graph.facebook.com/${version}/${encodeURIComponent(row.provider_account_id)}/media`);mediaUrl.searchParams.set("fields","id,timestamp,media_type");mediaUrl.searchParams.set("limit","25");
  const response=await fetch(mediaUrl,{headers});const body=await response.json().catch(()=>({}));if(!response.ok)return {ready:false,status:response.status,analyticsBlocked:response.status===401||response.status===403,error:"INSTAGRAM_MEDIA_PREFLIGHT_REJECTED"};
  const candidates=(Array.isArray(body.data)?body.data:[]).filter((item)=>typeof item?.id==="string");const metrics=["views","reach","saved","shares","total_interactions"];const diagnostics=[];
  for(const media of candidates){for(const metric of metrics){const probe=new URL(`https://graph.facebook.com/${version}/${encodeURIComponent(media.id)}/insights`);probe.searchParams.set("metric",metric);const checked=await fetch(probe,{headers});const payload=await checked.json().catch(()=>({}));const value=Number(payload?.data?.[0]?.values?.[0]?.value);diagnostics.push({metric,status:checked.status});if(checked.ok&&Number.isFinite(value))return {ready:true,remotePostId:media.id,source:`INSTAGRAM_MEDIA_INSIGHTS_${metric.toUpperCase()}_PROBE`,pagination:Boolean(body.paging?.next),candidatesChecked:candidates.length,metric,value};}}
  return {ready:false,status:diagnostics.at(-1)?.status||404,analyticsBlocked:diagnostics.some((item)=>item.status===401||item.status===403),error:"INSTAGRAM_NO_EXISTING_MEDIA_WITH_REAL_INSIGHTS",candidatesChecked:candidates.length,diagnostics};
}
async function linkedInPreflight(sql, marker, profileId, env) {
  if(!await ownedProfile(sql,marker,profileId)) throw new Error("QA_PROFILE_SCOPE_MISMATCH"); const row=await connection(sql,profileId,"LINKEDIN",env); const headers={authorization:`Bearer ${row.accessToken}`,accept:"application/json","Linkedin-Version":linkedinVersion(env),"X-Restli-Protocol-Version":"2.0.0"}; const info=row.metadata&&typeof row.metadata==="object"?row.metadata:{};
  if(info.accountType==="ORGANIZATION"){const url=new URL("https://api.linkedin.com/rest/organizationalEntityShareStatistics");url.searchParams.set("q","organizationalEntity");url.searchParams.set("organizationalEntity",`urn:li:organization:${row.provider_account_id}`);url.searchParams.set("shares",`List(${KNOWN_LINKEDIN_7F})`);const response=await fetch(url,{headers});return {ready:response.ok,status:response.status,analyticsBlocked:response.status===401||response.status===403,remotePostId:response.ok?KNOWN_LINKEDIN_7F:null,source:"ORGANIZATION_ANALYTICS_PROBE",accountType:"ORGANIZATION"};}
  if(!Array.isArray(row.permissions)||!row.permissions.includes("r_member_postAnalytics"))return {ready:false,status:403,analyticsBlocked:true,error:"MISSING_r_member_postAnalytics",source:"LOCAL_SCOPE_GATE",accountType:"MEMBER"};
  const url=new URL("https://api.linkedin.com/rest/memberCreatorPostAnalytics");url.searchParams.set("q","entity");url.searchParams.set("entity",`(share:${KNOWN_LINKEDIN_7F})`);url.searchParams.set("queryType","IMPRESSION");url.searchParams.set("aggregation","TOTAL");const response=await fetch(url,{headers});return {ready:response.ok,status:response.status,analyticsBlocked:response.status===401||response.status===403,remotePostId:response.ok?KNOWN_LINKEDIN_7F:null,source:"MEMBER_ANALYTICS_PROBE",accountType:"MEMBER"};
}
...
export default { async fetch(request,env){if(request.method!=="POST")return json({error:"METHOD_NOT_ALLOWED"},405);if(!sameSecret(request.headers.get("x-analytics7h-token")||"",env.ANALYTICS7H_TOKEN||""))return json({error:"FORBIDDEN"},403);let body;try{body=await request.json();}catch{return json({error:"INVALID_JSON"},400);}if(!validMarker(body?.marker)||!env.DATABASE_URL)return json({error:"INVALID_REQUEST"},400);const sql=neon(env.DATABASE_URL);try{
  ...
  if(body.action==="instagram-preflight")return json(await instagramPreflight(sql,body.marker,body.profileId,env));
