...
const emailPattern = /^onboarding-completion-smoke-([0-9]{10,32})-(primary|other)@example\.invalid$/;
...
async function users(sql: ReturnType<typeof neon>, marker?: string) {
  const rows = await sql`
    ...
    where lower(coalesce(to_jsonb(u)->>'email','')) like 'onboarding-completion-smoke-%@example.invalid'
...
async function state(sql: ReturnType<typeof neon>, marker: string) {
  ...
  const row = (await sql`
    select
      ...
      (select count(*)::int from public.platform_admin_audit a where a.actor_auth_user_id=any(${ids}::text[]) and a.metadata->>'phase'='FASE_5A') as qa_provision_audit,
      (select count(*)::int from public.platform_admin_audit a where a.actor_auth_user_id=any(${ids}::text[]) and a.metadata->>'phase'='FASE_5B') as qa_completion_audit,
      (select count(*)::int from neon_auth.user u where u.id::text=any(${ids}::text[]) and coalesce(u.banned,false)) as qa_banned,
      (select count(*)::int from public.platform_admin_audit a where a.metadata->>'phase' in ('FASE_5A','FASE_5B') and a.actor_auth_user_id in (select id::text from neon_auth.user u where lower(coalesce(to_jsonb(u)->>'email','')) like 'onboarding-completion-smoke-%@example.invalid')) as recognized_qa_audit,
  ...
  return {
    ...
    qaOperations: Number(row.qa_operations || 0), qaProvisionAudit: Number(row.qa_provision_audit || 0),
    qaCompletionAudit: Number(row.qa_completion_audit || 0), qaBanned: Number(row.qa_banned || 0),
...
async function cleanup(sql: ReturnType<typeof neon>, marker?: string) {
  ...
  if (ids.length) {
    ...
    await sql`delete from public.platform_admin_audit where actor_auth_user_id=any(${ids}::text[]) and metadata->>'phase' in ('FASE_5A','FASE_5B')`;
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    try {
      ...
      if (body.action === "ban-other") {
        const marked = await users(sql, body.marker);
        const other = marked.find((row) => row.match?.[2] === "other");
        if (!other) return json({ error: "QA_USER_NOT_FOUND" }, 404);
        await sql`update neon_auth.user set banned=true where id::text=${other.id}`;
