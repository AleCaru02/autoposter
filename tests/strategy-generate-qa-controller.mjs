...
import { makeQaProviderKey, parseQaProviderKey, openAiCallType, allowedProviderCallType, fakeOpenAiPlan, safeProviderRecord, technicalPersistenceFailureBody } from "./strategy-generate-provider-harness.mjs";
import { QA_ACTION_PROVIDER, QA_ACTION_BARRIER, QA_ACTION_BACKGROUND, json, sameSecret, validMarker, validScenario, previewRequest, sqlFor, currentSql, requestBodyText } from "./strategy-generate-qa-common.mjs";
import { assertQaProfile, state, cleanup, cleanupMarkerUser } from "./strategy-generate-qa-state.mjs";
import { setupProfile, configureEntitlement, resetProfile, prepareAutopilot, usage, providerCalls, releaseBarrier, backgroundStatus } from "./strategy-generate-qa-fixtures.mjs";
...
async function interceptedFetch(input,init={}){
  ...
  if(url.hostname==="api.openai.com"){
    ...
    const callType=openAiCallType(url.toString(),init);await sql`insert into public.platform_admin_audit(actor_auth_user_id,action,target_type,target_id,metadata) values (${`STRATEGY_GENERATE_QA_${c.marker}`},${QA_ACTION_PROVIDER},'QA_PROVIDER',${c.operationId},${JSON.stringify(safeProviderRecord(c,callType||"UNKNOWN"))}::jsonb)`;
  ...
  const raw=await requestBodyText(input,init);if(technicalPersistenceFailureBody(raw))return json({error:"STRATEGY_GENERATE_QA_TECHNICAL_LEDGER_FAILURE"},503);return nativeFetch(input,init);
}
...
async function control(request,env){
  ...
  if(!sameSecret(request.headers.get("x-strategy-generate-qa-token")||"",env.STRATEGY_GENERATE_QA_TOKEN||""))return json({error:"FORBIDDEN"},403);
  let body;try{body=await request.json();}catch{return json({error:"INVALID_JSON"},400);}const marker=body?.marker;if(!validMarker(marker)||request.headers.get("x-strategy-generate-qa-marker")!==marker)return json({error:"INVALID_MARKER"},400);const sql=sqlFor(env.DATABASE_URL);
  try{switch(body.action){
    case"preflight":case"state":return json(await state(sql,marker));
    case"instrumentation-status":return json({armed:true,previewOnly:true,persistedCounter:"platform_admin_audit",promptLogging:false,supports:["STRATEGIST","PLANNER","PROVIDER_FAILURE","LATENCY","BARRIER","TECHNICAL_LEDGER_FAILURE"]});
  ...
  }}catch(reason){console.error("strategy-generate-qa-controller",reason instanceof Error?reason.message:"unknown");return json({error:"CONTROLLER_FAILED",detail:reason instanceof Error?reason.message:"unknown"},500);}
}
async function productRoute(request,env,ctx){
  if(!previewRequest(request))return json({error:"RUNTIME_VERIFIER_NOT_CERTIFIED"},403);const marker=request.headers.get("x-strategy-generate-qa-marker")||"";const scenario=request.headers.get("x-strategy-generate-qa-scenario")||"";
  if(!sameSecret(request.headers.get("x-strategy-generate-qa-token")||"",env.STRATEGY_GENERATE_QA_TOKEN||"")||!validMarker(marker)||!validScenario(scenario))return json({error:"RUNTIME_VERIFIER_NOT_CERTIFIED"},403);
  ...
  const operationId=`strategy-${scenario}`;const fakeKey=makeQaProviderKey({marker,profileId,scenario,operationId});const observedCtx={waitUntil(promise){ctx.waitUntil(Promise.resolve(promise).then(()=>sql`insert into public.platform_admin_audit(actor_auth_user_id,action,target_type,target_id,metadata) values (${`STRATEGY_GENERATE_QA_${marker}`},${QA_ACTION_BACKGROUND},'QA_BACKGROUND',${operationId},${JSON.stringify({marker,profileId,scenario,status:"COMPLETED"})}::jsonb)`));}};return productWorker.fetch(request,{...env,OPENAI_API_KEY:fakeKey},observedCtx);
}
export default{async fetch(request,env,ctx){if(!env.DATABASE_URL||!env.STRATEGY_GENERATE_QA_TOKEN)return json({error:"RUNTIME_VERIFIER_NOT_CERTIFIED"},503);sqlFor(env.DATABASE_URL);const path=new URL(request.url).pathname;if(path==="/__qa/control")return control(request,env);if(path==="/api/editorial-agents/strategy-plan"||path==="/api/autopilot/run")return productRoute(request,env,ctx);return json({error:"RUNTIME_VERIFIER_NOT_CERTIFIED"},404);}};
