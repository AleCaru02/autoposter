...
const fase7c = workflow.includes("tests/fase7c-runtime.mjs");
...
assert.match(workflow, /(?:EPHEMERAL_CONTROLLER_VERSION_ID|FASE7C_CONTROLLER_VERSION_ID)/, "ephemeral version ID comparison missing");
assert.match(workflow, /(?:AUDIT|FASE7C)_PREVIEW_ISOLATION:\s*PASS/, "preview isolation assertion missing");
assert.match(workflow, /production deployment (?:set )?changed during (?:Audit runtime|verifier)/, "deployment immutability comparison missing");
assert.match(workflow, /if:\s*always\(\)/, "always cleanup missing");
assert.match(workflow, /shred -u \.(?:audit|fase7c)-runtime-secrets\.env/, "local token material cleanup missing");
...
assert.match(controller, /(?:recognizedSmokeEmail|emailPattern)/, "exact smoke fixture recognition missing");
assert.match(controller, /(?:audit-smoke-|fase7c-)/, "smoke fixture namespace missing");
