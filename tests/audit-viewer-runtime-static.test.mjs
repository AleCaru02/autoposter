...
assert.match(workflow, /(?:AUDIT_PREVIEW_ISOLATION|IMAGE_GENERATE_PREVIEW_ISOLATION):\s*PASS/, "preview isolation assertion missing");
...
assert.match(workflow, /shred -u \.(?:audit|image-generate)-runtime-secrets\.env/, "local token material cleanup missing");
