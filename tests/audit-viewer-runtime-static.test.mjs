...
assert.match(workflow, /(?:AUDIT_PREVIEW_ISOLATION|STRATEGY_GENERATE_PREVIEW_ISOLATION):\s*PASS/, "preview isolation assertion missing");
...
assert.match(workflow, /shred -u \.(?:audit|strategy-generate)-runtime-secrets\.env/, "local token material cleanup missing");
