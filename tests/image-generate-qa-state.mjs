import { QA_ACTION_PROVIDER, QA_ACTION_BARRIER, QA_ACTION_BACKGROUND, QA_EMAIL } from "./image-generate-qa-common.mjs";
...
async function qaUsers(sql, marker = null) {
  const rows = await sql`
    ...
    where lower(coalesce(to_jsonb(u)->>'email','')) like 'image-qa-%@example.invalid'
...
export async function state(sql, marker) {
  ...
  const pattern = `image-qa-${marker}-[ab]@example.invalid`;
  const rows = await sql`
    select
      ...
      (select count(*)::int from public.assets a where a.profile_id in (select id from public.profiles where owner_auth_user_id = any(${userIds}::text[]))) as qa_assets,
      (select count(*)::int from public.content_strategies s where s.profile_id in (select id from public.profiles where owner_auth_user_id = any(${userIds}::text[]))) as qa_strategies,
  ...
  return {
    ...
    qaStrategies: Number(row.qa_strategies || 0),
...
export async function cleanupMarkerUser(sql, marker, suffix) {
  ...
  const expected = `image-qa-${marker}-${normalized}@example.invalid`;
...
export async function cleanup(sql, marker, allRecognized = false) {
  ...
  if (allRecognized) {
    await sql`delete from public.platform_admin_audit where action in (${QA_ACTION_PROVIDER},${QA_ACTION_BARRIER},${QA_ACTION_BACKGROUND}) and actor_auth_user_id like 'IMAGE_GENERATE_QA_%'`;
  } else {
