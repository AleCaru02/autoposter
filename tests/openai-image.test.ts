...
import { buildImagePrompt, generateOpenAIImage, imageSizeForFormat } from "../api/_lib/openai-image.js";
...
const fetcher = (async (url: string | URL | Request, init?: RequestInit) => {
  ...
  return new Response(JSON.stringify({
    created: 1,
    data: [{ b64_json: Buffer.from("fake-png").toString("base64"), revised_prompt: "Professional property image" }],
    usage: { input_tokens: 50, output_tokens: 1200, total_tokens: 1250 },
  }), { status: 200, headers: { "x-request-id": "req_image_test", "content-type": "application/json" } });
}) as typeof fetch;
...
assert.ok(prompt.includes("non affermare fatti nuovi"));
...
assert.deepEqual(result.usage, { inputTokens: 50, outputTokens: 1200, totalTokens: 1250 });

console.log("PASS OpenAI image contract: esclusivamente gpt-image-2, qualità high, una immagine per richiesta, output PNG.");
