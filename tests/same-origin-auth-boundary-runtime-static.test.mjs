...
assert.match(runner, /const expectedCallbackHost = "neonauth\.us-west-2\.aws\.neon\.tech"/);
assert.match(runner, /sign-in.*social.*init/);
assert.match(runner, /\/auth\/oauth\/callback\/google/);
assert.match(runner, /handoffTokenPresent = Boolean\(handoff\.searchParams\.get\("token"\)\)/);
assert.match(runner, /assert\.equal\(pkcePresent, true, "Google OAuth PKCE challenge missing"\)/);
assert.match(runner, /NEON_MANAGED_AUTH_CENTRAL_OAUTH_CALLBACK/);
