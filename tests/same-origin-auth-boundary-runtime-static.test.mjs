...
assert.match(runner, /callbackStatus:/);
assert.match(runner, /callbackSameOrigin:/);
assert.match(runner, /callbackTokenPresent:/);
assert.match(runner, /providerStatus:/);
assert.match(runner, /providerErrorCode:/);
...
assert.match(controller, /\/api\/auth\/reset-password\/\$\{encodeURIComponent\(challenge\)\}/, "reset verifier must execute the emailed-link callback leg");
assert.match(controller, /callbackURL=\$\{encodeURIComponent\(callbackUrl\)\}/);
assert.match(controller, /redirect:\s*"manual"/);
assert.match(controller, /redirect\.origin === APP_BASE && redirect\.pathname === "\/reimposta-password"/);
assert.match(controller, /callbackTokenPresent/);
assert.match(controller, /providerErrorCode/);
