...
async function qaUsers(sql, marker = null) {
  const rows = await sql`
    ...
    where lower(coalesce(to_jsonb(u)->>'email','')) like 'ai-text-qa-%@example.invalid'
...
export async function state(sql, marker) {
  ...
  const pattern = `ai-text-qa-${marker}@example.invalid`;
  const rows = await sql`
    select
...
export async function cleanup(sql, marker, allRecognized = false) {
  ...
  if (allRecognized) {
    await sql`delete from public.platform_admin_audit where action in (${QA_ACTION_PROVIDER},${QA_ACTION_BARRIER},${QA_ACTION_BACKGROUND}) and actor_auth_user_id like 'AI_TEXT_QA_%'`;
  } else {
