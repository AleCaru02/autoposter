...
async function impersonationState(sql, marker) {
  ...
  if (!ids.length) return { sessions: [], summary: { total: 0, impersonated: 0, impersonatedByAdmin: 0 } };

  const rows = await sql`
    select
      coalesce(to_jsonb(s)->>'userId',to_jsonb(s)->>'user_id','') as user_id,
  ...
  const sessions = rows.map((row) => {
    ...
    return {
      currentKind: user?.kind || "OTHER",
      impersonated: Boolean(row.impersonated_by),
      impersonatedByKind: actor?.kind || (row.impersonated_by ? "OTHER" : null),
      actorMatchesAdmin: actor?.kind === "admin",
    };
  });
...
async function userState(sql, marker) {
  ...
  return {
    users: users.map((user) => ({
      ...
      banReasonPresent: Boolean(user.ban_reason),
      banExpiresPresent: Boolean(user.ban_expires),
    })),
  };
}
...
async function promote(sql, marker) {
  ...
  if (users.length !== 3 || users.some((user) => !allowed.has(user.email))) {
    return { status: 409, body: { error: "SMOKE_IDENTITY_SCOPE_MISMATCH", count: users.length } };
  }
  ...
  if (after.qaAdmins !== 1 || after.superAdmins !== 2) {
    return { status: 409, body: { error: "SMOKE_ADMIN_PROMOTION_POSTCONDITION", qaAdmins: after.qaAdmins, superAdmins: after.superAdmins } };
  }
...
async function cleanup(sql, marker) {
  ...
  const allowed = expectedEmails(marker);
  if (users.length > 3 || users.some((user) => !allowed.has(user.email))) {
    return { status: 409, body: { error: "SMOKE_CLEANUP_SCOPE_MISMATCH", count: users.length } };
  }
...
async function cleanupRecognizedResidue(sql, marker) {
  ...
  if (users.some((user) => !recognizedSmokeEmail.test(user.email))) {
    return { status: 409, body: { error: "STALE_SMOKE_SCOPE_MISMATCH" } };
  }
  ...
  if (after.recognizedQaUsers !== 0 || after.recognizedQaAdmins !== 0) {
    return { status: 409, body: { error: "STALE_SMOKE_CLEANUP_POSTCONDITION" } };
  }
...
export default {
  async fetch(request, env) {
    ...
    if (!["preflight", "state", "promote", "impersonation-state", "user-state", "cleanup", "cleanup-residue"].includes(body?.action)) {
    ...
    try {
      ...
      console.error("admin-impersonation-provider-preview-controller", reason instanceof Error ? reason.message : "unknown");
