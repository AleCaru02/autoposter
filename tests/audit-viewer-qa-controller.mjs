...
async function auditState(sql, marker) {
  ...
  const rows = await sql`
    select action, actor_auth_user_id, target_id, metadata
  ...
  return { events: rows.map((row) => ({
    action: row.action,
    actorKind: byId.get(String(row.actor_auth_user_id || ""))?.kind || "OTHER",
    targetKind: byId.get(String(row.target_id || ""))?.kind || "OTHER",
    provider: row.metadata?.provider || null,
    sessionBound: row.metadata?.sessionBound === true,
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      console.error("admin-impersonation-api-preview-controller", { failure: "CONTROLLER_FAILED" });
