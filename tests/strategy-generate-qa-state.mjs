import { QA_ACTION_PROVIDER, QA_ACTION_BARRIER, QA_ACTION_BACKGROUND, QA_EMAIL } from "./strategy-generate-qa-common.mjs";
...
async function qaUsers(sql, marker = null) {
  const rows = await sql`
    ...
    where lower(coalesce(to_jsonb(u)->>'email','')) like 'strategy-qa-%@example.invalid'
...
export async function state(sql, marker) {
  ...
  const pattern = `strategy-qa-${marker}-[ab]@example.invalid`;
  const rows = await sql`
    select
...
export async function cleanupMarkerUser(sql, marker, suffix) {
  ...
  const expected = `strategy-qa-${marker}-${normalized}@example.invalid`;
...
export async function cleanup(sql, marker, allRecognized = false) {
  ...
  if (allRecognized) {
    await sql`delete from public.platform_admin_audit where action in (${QA_ACTION_PROVIDER},${QA_ACTION_BARRIER},${QA_ACTION_BACKGROUND}) and actor_auth_user_id like 'STRATEGY_GENERATE_QA_%'`;
  } else {
