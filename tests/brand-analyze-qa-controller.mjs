...
import { assertQaProfile, state, cleanup, cleanupMarkerUser } from "./brand-analyze-qa-state.mjs";
...
async function control(request,env){
  ...
  try{switch(body.action){
    ...
    case"setup-profile":return json(await setupProfile(sql,marker,body.profileId));case"configure-entitlement":return json(await configureEntitlement(sql,marker,body.profileId,body.mode,body.limitValue));case"reset-profile":return json(await resetProfile(sql,marker,body.profileId));case"usage":return json(await usage(sql,marker,body.profileId));case"provider-calls":return json(await providerCalls(sql,marker,body.profileId||null,body.scenario||null));case"release-barrier":return json(await releaseBarrier(sql,marker,body.profileId,body.scenario,body.operationId));case"cleanup-user":return json(await cleanupMarkerUser(sql,marker,body.suffix));case"cleanup":return json(await cleanup(sql,marker,false));case"cleanup-residue":return json(await cleanup(sql,marker,true));default:return json({error:"INVALID_ACTION"},400);
