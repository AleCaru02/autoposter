...
async function state(sql, marker) {
  ...
  const usageDetail=await sql`select e.state,e.capability_key,e.metadata->>'execution_state' execution_state,e.metadata->>'release_reason' release_reason,e.metadata->>'provider_started_at' provider_started_at from public.capability_usage_events e join public.profiles p on p.id=e.profile_id join neon_auth.user u on u.id::text=p.owner_auth_user_id where lower(coalesce(to_jsonb(u)->>'email','')) like ${pattern} order by e.created_at`;
  return {qaUsers:users.length,recognizedQaUsers:all.length,profilesTotal:Number(counts.profiles_total||0),jobsTotal:Number(counts.jobs_total||0),qaProfiles:Number(counts.qa_profiles||0),qaContent:Number(counts.qa_content||0),qaAssets:Number(counts.qa_assets||0),qaJobs:Number(counts.qa_jobs||0),qaUsage:Number(counts.qa_usage||0),detail,usageDetail};
}
...
async function fixture(sql,marker,profileId){
  ...
  const generatedAt=new Date().toISOString();const plan={horizonDays:14,planningSummary:"QA unsupported-plan normalization",items:[{dayOffset:2,provider:"FACEBOOK",contentType:"CAROUSEL",intent:"TIP",topicDirection:"Racconta esclusivamente che l'attività offre consulenza editoriale responsabile alle piccole imprese italiane, con tono chiaro e senza promesse; non usare numeri, date, norme o fatti esterni",objective:"Informare",funnelStage:"AWARENESS"}]};
  ...
  const strategy={autopilotEnabled:true,approvalMode:"MANUAL_REVIEW",researchMode:"WEBSITE_ONLY",aiEconomics:{monthlyAiBudgetUsd:5,monthlyImageLimit:0,maxGenerationsPerDay:1,maxGenerationsPerWeek:1,generateImagesAfterApproval:true},aiStrategy,aiStrategyGeneratedAt:generatedAt,aiEditorialPlanGeneratedAt:generatedAt,aiEditorialPlan:plan};
