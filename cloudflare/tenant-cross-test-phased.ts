...
type IdentityInput = { email?: unknown; jwt?: unknown };
type FixtureInput = {
  profileId?: unknown;
  profileName?: unknown;
  membershipUserId?: unknown;
  brandId?: unknown;
};
type TestPayload = {
  mode?: unknown;
  a?: IdentityInput;
  b?: IdentityInput;
  fixtures?: { a?: FixtureInput; b?: FixtureInput };
};
type Fixture = {
  userId: string;
  jwt: string;
...
function validProfileName(value: unknown, label: "A" | "B") {
  return typeof value === "string" && new RegExp(`^Tenant QA ${label} [a-f0-9]{8}$`, "i").test(value) ? value : null;
}
...
function fixtureFromPayload(identityInput: IdentityInput | undefined, fixtureInput: FixtureInput | undefined, label: "A" | "B"): Fixture {
  const state: FixtureState = {};
  const auth = identity(identityInput, label, state);
  const profileId = validUuid(fixtureInput?.profileId);
  const profileName = validProfileName(fixtureInput?.profileName, label);
  const membershipUserId = validUuid(fixtureInput?.membershipUserId);
  const brandId = fixtureInput?.brandId == null ? null : validUuid(fixtureInput.brandId);
  if (!profileId || !profileName || !membershipUserId || (fixtureInput?.brandId != null && !brandId)) throw new Error(`FIXTURE_${label}_INVALID`);
  return { ...auth, profileId, profileName, membershipUserId, brandId };
}

function publicFixture(fixture: Fixture): FixtureInput {
  return {
    profileId: fixture.profileId,
    profileName: fixture.profileName,
    membershipUserId: fixture.membershipUserId,
...
async function ownAccessChecks(actor: Fixture, label: "A" | "B", checks: Check[]) {
  ...
  const ownProfilePatch = await query(actor.jwt, `profiles?id=eq.${encodeURIComponent(actor.profileId)}&select=id,name`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ name: ownChangedName }),
  });
  ...
  const restoreProfile = await query(actor.jwt, `profiles?id=eq.${encodeURIComponent(actor.profileId)}&select=id`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ name: actor.profileName }),
  });
  ...
  const ownBrandPatch = await query(actor.jwt, `brand_profiles?profile_id=eq.${encodeURIComponent(actor.profileId)}&select=profile_id,description`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ description: `private-${label}-updated` }),
  });
  ...
  const recreateBrand = await query(actor.jwt, "brand_profiles?select=profile_id,description", {
    method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ profile_id: actor.profileId, description: `private-${label}` }),
  });
...
async function crossReadChecks(actor: Fixture, target: Fixture, actorLabel: "A" | "B", targetLabel: "A" | "B", checks: Check[]) {
  ...
  } else check(checks, `${actorLabel}_cannot_read_${targetLabel}_brand_by_known_resource_id`, true);
}
...
async function crossWriteChecks(actor: Fixture, target: Fixture, actorLabel: "A" | "B", targetLabel: "A" | "B", checks: Check[]) {
  ...
  const profilePatch = await query(actor.jwt, `profiles?id=eq.${encodeURIComponent(target.profileId)}&select=id`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ name: changedName }),
  });
  const ownerProfile = await profileRows(target.jwt, target.profileId, "id,name");
  check(checks, `${actorLabel}_cannot_update_${targetLabel}_profile_by_direct_api`, mutationDenied(profilePatch) && ownerProfile.ok && ownerProfile.rows.length === 1 && ownerProfile.rows[0]?.name === target.profileName);
  ...
  const brandPatch = await query(actor.jwt, `brand_profiles?profile_id=eq.${encodeURIComponent(target.profileId)}&select=profile_id`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ description: `forbidden-${actorLabel}` }),
  });
  const ownerBrand = await ownRows(target.jwt, "brand_profiles", target.profileId, "profile_id,description");
  check(checks, `${actorLabel}_cannot_update_${targetLabel}_brand_by_direct_api`, mutationDenied(brandPatch) && ownerBrand.ok && ownerBrand.rows.length === 1 && ownerBrand.rows[0]?.description === `private-${targetLabel}`);
  ...
  check(checks, `${actorLabel}_cannot_delete_${targetLabel}_brand_by_direct_api`, mutationDenied(brandDelete) && brandAfterDelete.ok && brandAfterDelete.rows.length === 1);
  ...
  check(checks, `${actorLabel}_cannot_delete_${targetLabel}_profile_by_direct_api`, mutationDenied(profileDelete) && profileAfterDelete.ok && profileAfterDelete.rows.length === 1);

  const hostileBrandCreate = await query(actor.jwt, "brand_profiles?select=profile_id", {
    method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ profile_id: target.profileId, description: `forbidden-create-${actorLabel}` }),
  });
  const targetBrands = await ownRows(target.jwt, "brand_profiles", target.profileId, "profile_id,description");
  check(checks, `${actorLabel}_cannot_create_resource_with_${targetLabel}_profile_id`, mutationDenied(hostileBrandCreate) && targetBrands.ok && targetBrands.rows.length === 1 && targetBrands.rows[0]?.description === `private-${targetLabel}`);
}
...
async function membershipEscalationChecks(actor: Fixture, target: Fixture, actorLabel: "A" | "B", targetLabel: "A" | "B", checks: Check[]) {
  const joinTarget = await query(actor.jwt, "profile_members?select=profile_id,user_id,role", {
    method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ profile_id: target.profileId, user_id: actor.membershipUserId, role: "OWNER" }),
  });
  ...
  const addOtherUser = await query(actor.jwt, "profile_members?select=profile_id,user_id,role", {
    method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ profile_id: actor.profileId, user_id: target.membershipUserId, role: "OWNER" }),
  });
  ...
  const changeOwnRole = await query(actor.jwt, `profile_members?profile_id=eq.${encodeURIComponent(actor.profileId)}&user_id=eq.${encodeURIComponent(actor.membershipUserId)}&select=profile_id,user_id,role`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ role: "SUPER_ADMIN" }),
  });
  ...
  const moveOwnMembership = await query(actor.jwt, `profile_members?profile_id=eq.${encodeURIComponent(actor.profileId)}&user_id=eq.${encodeURIComponent(actor.membershipUserId)}&select=profile_id,user_id,role`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ profile_id: target.profileId }),
  });
  ...
  const replaceOwnerUser = await query(actor.jwt, `profiles?id=eq.${encodeURIComponent(actor.profileId)}&select=id,owner_user_id`, {
    method: "PATCH", headers: { prefer: "return=representation" }, body: JSON.stringify({ owner_user_id: target.membershipUserId }),
  });
  const actorProfileAfterOwnerReplace = await profileRows(actor.jwt, actor.profileId, "id,owner_user_id");
  check(checks, `${actorLabel}_cannot_replace_server_derived_owner_user_id`, mutationDenied(replaceOwnerUser) && actorProfileAfterOwnerReplace.ok && actorProfileAfterOwnerReplace.rows.length === 1 && actorProfileAfterOwnerReplace.rows[0]?.owner_user_id === actor.membershipUserId);
}
...
async function phaseResult(phase: string, run: (checks: Check[]) => Promise<void>) {
  const checks: Check[] = [];
  let error: string | null = null;
  try { await run(checks); }
  catch (reason) { error = reason instanceof Error ? reason.message : "TENANT_PHASE_FAILED"; }
  const ready = error === null && checks.length > 0 && checks.every((item) => item.pass);
  return json({ service: "post-automatici", ready, phase, checks, error }, ready ? 200 : 503);
}
...
export async function handleTenantCrossTest(request: Request, env: CrossTenantEnv) {
  ...
  if (payload.mode === "prepare") {
    ...
    try {
      ...
      return json({ service: "post-automatici", ready: true, phase: "prepare", fixtures: { a: publicFixture(a), b: publicFixture(b) } });
    } catch (reason) {
      try { await cleanupFixture(sql, aState); await cleanupFixture(sql, bState); } catch { /* fallback workflow cleanup still runs */ }
      return json({ service: "post-automatici", ready: false, phase: "prepare", error: reason instanceof Error ? reason.message : "TENANT_PREPARE_FAILED" }, 503);
    }
  }
  ...
  let b: Fixture;
  try {
    a = fixtureFromPayload(payload.a, payload.fixtures?.a, "A");
    b = fixtureFromPayload(payload.b, payload.fixtures?.b, "B");
  } catch (reason) {
    return json({ service: "post-automatici", ready: false, error: reason instanceof Error ? reason.message : "FIXTURE_INPUT_INVALID" }, 400);
  }

  if (payload.mode === "phase-1") return phaseResult("phase-1", async (checks) => {
  ...
  if (payload.mode === "phase-2a") return phaseResult("phase-2a", async (checks) => {
    await crossWriteChecks(a, b, "A", "B", checks);
    await membershipEscalationChecks(a, b, "A", "B", checks);
  });

  if (payload.mode === "phase-2b") return phaseResult("phase-2b", async (checks) => {
