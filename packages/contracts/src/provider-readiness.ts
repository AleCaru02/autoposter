import { z } from 'zod';
import { PostVariantSchema, SocialPlatformSchema } from './index.js';

export const ProviderKeySchema = z.enum([
  'meta','facebook','instagram','linkedin','google_business_profile','telegram','stripe','openai',
]);
...
export const ConnectionHealthStatusSchema = z.enum([
  'CONNECTED','DEGRADED','EXPIRING','EXPIRED','REAUTH_REQUIRED','PERMISSION_MISSING','RATE_LIMITED','PROVIDER_ERROR','DISCONNECTED',
]);
...
export const ProviderAccountSchema = z.object({
  ...
  health: ConnectionHealthStatusSchema.default('CONNECTED'),
...
export const ProviderConnectionSchema = z.object({
  ...
  providerSubjectId: z.string().optional(),
  grantedScopes: z.array(z.string()).default([]),
  missingPermissions: z.array(z.string()).default([]),
  tokenExpiresAt: z.string().datetime().optional(),
...
export const NormalizedPublishPayloadSchema = z.object({
  ...
  media: z.array(PublishMediaSchema).default([]),
...
export const ProviderValidationIssueSchema = z.object({
  code: z.string(),
...
export const PublishDryRunSchema = z.object({
  ...
  payload: NormalizedPublishPayloadSchema,
...
export const ProviderPublishResultSchema = z.object({
...
export const OAuthExchangeResultSchema = z.object({
  providerSubjectId: z.string(),
  grantedScopes: z.array(z.string()).default([]),
  expiresAt: z.string().datetime().optional(),
  hasRefreshCredential: z.boolean().default(false),
...
export const AICapabilitySchema = z.enum([
  'TEXT_CHEAP','TEXT_STANDARD','TEXT_REASONING','STRUCTURED_OUTPUT','EMBEDDING','VISION','IMAGE_GENERATION','IMAGE_EDIT','WEB_RESEARCH',
]);
export type AICapability = z.infer<typeof AICapabilitySchema>;

export const AIErrorCodeSchema = z.enum([
  'TIMEOUT','MALFORMED_OUTPUT','VALIDATION_FAILURE','RATE_LIMIT','PROVIDER_UNAVAILABLE','SAFETY_REJECTION','EMPTY_RESPONSE','PARTIAL_RESPONSE','COST_LIMIT',
]);
export type AIErrorCode = z.infer<typeof AIErrorCodeSchema>;

export const AIExecutionPolicySchema = z.object({
  timeoutMs: z.number().int().positive(),
  maxAttempts: z.number().int().min(1).max(3).default(2),
  maxCostMicrounits: z.number().int().nonnegative().optional(),
  retryableErrors: z.array(AIErrorCodeSchema).default(['TIMEOUT','RATE_LIMIT','PROVIDER_UNAVAILABLE']),
});
export type AIExecutionPolicy = z.infer<typeof AIExecutionPolicySchema>;

const SourceEvidenceSchema = z.object({
  sourceType: z.enum(['WEBSITE','DOCUMENT','USER_CONFIRMED','USER_INPUT','PUBLIC_RESEARCH','SOCIAL','SYSTEM_INFERENCE']),
...
export const BrandIntelligenceOutputSchema = z.object({
  brandName: z.string(), industry: z.string(), description: z.string(), services: z.array(z.string()), products: z.array(z.string()),
  audiences: z.array(z.string()), differentiators: z.array(z.string()), toneRules: z.array(z.string()), allowedClaims: z.array(z.string()), forbiddenClaims: z.array(z.string()), evidence: z.array(SourceEvidenceSchema),
});
...
export const PlatformVariantOutputSchema = PostVariantSchema;
...
export const StructuredOutputSchemas = {
  brand_intelligence: BrandIntelligenceOutputSchema,
  content_strategy: ContentStrategyOutputSchema,
  topic_research: TopicResearchOutputSchema,
  core_concept: CoreConceptOutputSchema,
  platform_variant: PlatformVariantOutputSchema,
  caption: CaptionOutputSchema,
  visual_brief: VisualBriefOutputSchema,
  qa: QAOutputSchema,
  fact_check: FactCheckOutputSchema,
  competitor_analysis: CompetitorAnalysisOutputSchema,
  analytics_insight: AnalyticsInsightOutputSchema,
  image_prompt: ImagePromptOutputSchema,
  document_intelligence: DocumentIntelligenceOutputSchema,
} as const;
...
export interface SocialProviderV2 {
  readonly provider: ProviderKey;
  readonly platform: z.infer<typeof SocialPlatformSchema>;
...
export interface WebhookSignatureVerifier {
  readonly provider: ProviderKey;
  verify(input:{rawBody:string;headers:Record<string,string|undefined>;nowMs:number}):{valid:boolean;timestampMs?:number;reason?:string};
}
