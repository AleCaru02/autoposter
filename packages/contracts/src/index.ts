...
export const QualityScoreSchema = z.object({
  ...
  duplicateSignals: z.object({
    ...
    sameTenant: z.number().min(0).max(1),
    crossTenantTemplate: z.number().min(0).max(1),
  }),
});
...
export const BrandContextCompactSchema = z.object({
  ...
  lockedFacts: z.record(z.string(), z.unknown()).default({}),
...
export const PostVariantSchema = z.object({
  ...
  visualBrief: z.record(z.string(), z.unknown()).default({}),
...
export interface ModelRoute {
  task: string;
  tier: 'simple' | 'medium' | 'complex' | 'image' | 'embedding';
...
export interface ModelRouter {
  resolve(task: string, context: { risk: 'low' | 'medium' | 'high'; budgetRemainingMicrounits?: number }): Promise<ModelRoute>;
}

export interface GenerationContext {
  tenantId: string;
  brand: BrandContextCompact;
...
export interface AIOrchestrator {
  generateCoreConcept(context: GenerationContext): Promise<CoreConcept>;
  generatePlatformVariants(context: GenerationContext, concept: CoreConcept, platforms: SocialPlatform[]): Promise<PostVariant[]>;
  scoreAndValidate(context: GenerationContext, concept: CoreConcept, variants: PostVariant[]): Promise<QualityScore>;
}

export interface SocialConnectionHealth {
  status: 'connected' | 'expiring' | 'expired' | 'reauth_required' | 'permission_error' | 'disabled';
  expiresAt?: string;
