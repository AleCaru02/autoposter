...
const context = (correlationId: string, tenantId = 'tenant-a') => ({
  tenantId,
  brand: {
    brandName: 'Demo Studio Milano',
    description: 'Servizi professionali con sede locale',
    industry: 'servizi professionali',
    locations: ['Milano'],
    audiences: ['PMI locali'],
    services: ['consulenza'],
    products: [],
    differentiators: ['processo trasparente'],
    valuePropositions: ['piano operativo misurabile'],
    toneRules: ['chiaro', 'specifico'],
    bannedWords: ['migliore'],
    allowedClaims: ['Sede a Milano'],
    forbiddenClaims: ['risultato garantito'],
    ctaPreferences: ['Prenota una consulenza'],
    palette: ['#0F766E'],
    contentThemes: ['metodo', 'consulenza locale'],
...
describe('EditorialPipelineMock', () => {
  it('runs concept → variants → quality → manual approval → scheduling', async () => {
    ...
    const run = await pipeline.prepare({
      context: context('manual-1'),
      platforms: ['facebook', 'instagram', 'linkedin', 'google_business_profile'],
      approvalMode: 'manual',
  ...
  it('replays the same tenant/correlation prepare operation without duplicating a run', async () => {
    const pipeline = new EditorialPipelineMock();
    const input = {
      context: context('same-correlation'),
  ...
  it('blocks approval entirely when the quality gate is below threshold', async () => {
    ...
    const lowQuality = {
      ...
      scoreAndValidate: async (ctx: Parameters<typeof base.scoreAndValidate>[0], concept: Parameters<typeof base.scoreAndValidate>[1], variants: Parameters<typeof base.scoreAndValidate>[2]) => ({
        ...(await base.scoreAndValidate(ctx, concept, variants)),
        factConfidence: 0.2,
      }),
    };
    const pipeline = new EditorialPipelineMock(lowQuality);
    const run = await pipeline.prepare({
      context: context('low-quality'),
      platforms: ['instagram'],
      approvalMode: 'manual',
      scheduledAt: '2026-08-10T10:00:00.000Z',
      createdAt: '2026-08-09T10:00:00.000Z',
