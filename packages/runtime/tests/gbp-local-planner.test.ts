...
describe('GbpLocalContentPlanner', () => {
  it('skips GBP when no verified location exists', () => {
    const result = planner.plan(
      { ...context, hasVerifiedLocation: false, locationName: undefined },
  ...
  it('uses a native local variant when concept and CTA have local fit', () => {
    const result = planner.plan(context, {
      topic: 'Consulenza per attività di Milano',
      objective: 'local lead generation',
      ctaIntent: 'Prenota una visita',
      factualClaims: ['Sede Milano'],
      hasLocalRelevance: true,
      hasDateBoundEvent: false,
      hasOfferTerms: false,
    });
  ...
  it('creates a separate local concept when the main idea is non-local but tenant context is sufficient', () => {
    const result = planner.plan(context, {
      topic: 'Come organizzare un processo interno',
      objective: 'brand authority',
      ctaIntent: 'Leggi il metodo',
      factualClaims: [],
  ...
  it('downgrades unsupported event/offer requests instead of inventing dates or terms', () => {
    const event = planner.plan(context, {
      topic: 'Incontro locale',
    ...
    const offer = planner.plan(context, {
      topic: 'Promozione locale',
      objective: 'local conversion',
      ctaIntent: 'Contattaci',
      factualClaims: [],
      hasLocalRelevance: true,
      hasDateBoundEvent: false,
      hasOfferTerms: false,
  ...
  it('selects event or offer only when supporting facts are present', () => {
    const event = planner.plan(context, {
      topic: 'Open day Milano',
      objective: 'local awareness',
      ctaIntent: 'Prenota visita',
      factualClaims: ['Data confermata'],
      hasLocalRelevance: true,
      hasDateBoundEvent: true,
      hasOfferTerms: false,
    });
    ...
    const offer = planner.plan(context, {
      topic: 'Offerta consulenza Milano',
      objective: 'local conversion',
      ctaIntent: 'Contattaci',
      factualClaims: ['Termini offerta confermati'],
      hasLocalRelevance: true,
      hasDateBoundEvent: false,
      hasOfferTerms: true,
    });
