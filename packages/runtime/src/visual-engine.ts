...
const isoWeekAgo=(current:string)=>new Date(new Date(current).getTime()-7*86400000).toISOString();
...
export class DeterministicAssetClassifier implements AssetClassifier {
  classify(input:{filename:string;mimeType:string;width:number|null;height:number|null}):AssetClassification {
    ...
    const quality=pixels>=2_000_000?.95:pixels>=900_000?.86:pixels>0?.72:input.mimeType==='application/pdf'?.8:.68;
...
export const templateCatalog:Record<TemplateKey,{label:string;visualTypes:VisualType[]}>= {
  ...
  split_layout:{label:'Split Layout',visualTypes:['photo_plus_overlay','branded_card']},
...
export const deriveVisualTemplateProfile=(tenantId:string):VisualTemplateProfile=>{
  const seed=hashNumber(tenantId);
  const all=Object.keys(templateCatalog) as TemplateKey[];
  const start=seed%all.length;
  return{preferredTemplates:[all[start]!,all[(start+3)%all.length]!,all[(start+6)%all.length]!],spacing:(['compact','balanced','airy'] as const)[seed%3]!,imageRatio:'adaptive',textDensity:(['low','medium','high'] as const)[Math.floor(seed/3)%3]!,logoPosition:(['top_left','top_right','bottom_left','bottom_right'] as const)[Math.floor(seed/7)%4]!,borderStyle:(['none','soft','strong'] as const)[Math.floor(seed/11)%3]!,ctaStyle:(['pill','underline','boxed','minimal'] as const)[Math.floor(seed/13)%4]!,variantSeed:seed};
};
...
export class DeterministicVisualDirector {
  direct(input:{tenantId:string;platform:VisualPlatform;format:VisualFormat;topic:string;brief:VisualBrief;selection:AssetSelectionResult;profile:VisualTemplateProfile;recentFingerprints:string[]}):VisualDirection {
    ...
    const fallback=compatible[0]??'minimal_brand_card';
    let templateKey=ordered[0]??fallback;
    const shape=headlineShape(input.brief.headline);
    for(const candidate of ordered){
      const repeated=input.recentFingerprints.some((fingerprint)=>fingerprint.startsWith(`${candidate}|${visualType}|`)&&fingerprint.includes(`|${shape}|`));
      if(!repeated){templateKey=candidate;break}
    }
...
const rgb=(hex:string):[number,number,number]=>{const clean=hex.replace('#','');const full=clean.length===3?clean.split('').map((char)=>char+char).join(''):clean;const parsed=Number.parseInt(full,16);const value=Number.isFinite(parsed)?parsed:0;return[(value>>16)&255,(value>>8)&255,value&255]};
const linearChannel=(channel:number)=>{const scaled=channel/255;return scaled<=.03928?scaled/12.92:Math.pow((scaled+.055)/1.055,2.4)};
const luminance=(hex:string)=>{const channels=rgb(hex);return .2126*linearChannel(channels[0])+.7152*linearChannel(channels[1])+.0722*linearChannel(channels[2])};
export const contrastRatio=(a:string,b:string)=>{const first=luminance(a),second=luminance(b);return (Math.max(first,second)+.05)/(Math.min(first,second)+.05)};
const wrap=(text:string,max:number)=>{const tokens=text.trim().split(/\s+/).filter(Boolean);const lines:string[]=[];let line='';for(const token of tokens){const next=line?`${line} ${token}`:token;if(next.length>max&&line){lines.push(line);line=token}else line=next}if(line)lines.push(line);return lines};
...
export class DeterministicGraphicRenderer {
  validate(spec:RenderSpec):VisualQaIssue[]{
    ...
    const bg=spec.palette[0]??'#111111';
    const fg=spec.palette[1]??'#ffffff';
    if(spec.headline.length>72)issues.push({code:'HEADLINE_TOO_LONG',affectedComponent:'headline',severity:'error',repairAction:'shorten_headline'});
    if(spec.supportingText.length>190)issues.push({code:'BODY_TOO_LONG',affectedComponent:'supporting_text',severity:'error',repairAction:'shorten_supporting_text'});
    if(spec.cta.length>40)issues.push({code:'CTA_TOO_LONG',affectedComponent:'cta',severity:'warning',repairAction:'shorten_cta'});
    if(contrastRatio(bg,fg)<4.5)issues.push({code:'LOW_CONTRAST',affectedComponent:'contrast',severity:'error',repairAction:'use_accessible_palette'});
    const lower=`${spec.headline} ${spec.supportingText}`.toLowerCase();
    for(const claim of spec.forbiddenClaims.filter(Boolean)){if(lower.includes(claim.toLowerCase()))issues.push({code:'FORBIDDEN_FACT_CLAIM',affectedComponent:'fact_claim',severity:'blocker',repairAction:'remove_unverified_claim'})}
    return issues;
  }
  render(spec:RenderSpec):RenderResult {
    const preset=formatPresets[spec.format];
    const issues=this.validate(spec);
    const bg=spec.palette[0]??'#111827',fg=spec.palette[1]??'#ffffff',accent=spec.palette[2]??'#f59e0b';
    const safe=Math.round(preset.width*.07);
    const headlineLines=wrap(spec.headline,spec.format==='landscape'?34:26).slice(0,4);
    const bodyLines=wrap(spec.supportingText,spec.format==='landscape'?55:38).slice(0,5);
    const photo=spec.assetDataUri?`<image href="${escapeXml(spec.assetDataUri)}" x="0" y="0" width="${preset.width}" height="${preset.height}" preserveAspectRatio="xMidYMid slice"/>`:'';
    const overlay=spec.assetDataUri?`<rect width="100%" height="100%" fill="${bg}" opacity="0.48"/>`:'';
    const imageMode=['photo_full_bleed','photo_text_overlay','promotional','local_gbp'].includes(spec.templateKey);
    const split=spec.templateKey==='split_layout';
    const cardX=split?Math.round(preset.width*.52):safe;
    const cardW=split?Math.round(preset.width*.41):preset.width-safe*2;
    const contentY=Math.round(preset.height*.24);
    const logoX=spec.logoPosition.includes('right')?preset.width-safe:safe;
    const logoY=spec.logoPosition.includes('bottom')?preset.height-safe:Math.round(safe*.9);
    const logo=spec.logoDataUri&&spec.logoPosition!=='hidden'?`<image href="${escapeXml(spec.logoDataUri)}" x="${spec.logoPosition.includes('right')?logoX-160:logoX}" y="${spec.logoPosition.includes('bottom')?logoY-64:logoY}" width="160" height="64" preserveAspectRatio="xMidYMid meet"/>`:'';
    const headlineSvg=headlineLines.map((line,index)=>`<text x="${cardX}" y="${contentY+index*68}" font-family="${escapeXml(spec.fontFamily)}" font-size="58" font-weight="760" fill="${fg}">${escapeXml(line)}</text>`).join('');
    const bodyStart=contentY+headlineLines.length*68+36;
    const bodySvg=bodyLines.map((line,index)=>`<text x="${cardX}" y="${bodyStart+index*42}" font-family="${escapeXml(spec.fontFamily)}" font-size="30" fill="${fg}" opacity="0.94">${escapeXml(line)}</text>`).join('');
    const ctaY=Math.min(preset.height-safe-54,bodyStart+bodyLines.length*42+62);
    const cta=`<rect x="${cardX}" y="${ctaY-42}" width="${Math.min(cardW,Math.max(190,spec.cta.length*20+62))}" height="62" rx="31" fill="${accent}"/><text x="${cardX+28}" y="${ctaY}" font-family="${escapeXml(spec.fontFamily)}" font-size="25" font-weight="700" fill="#111111">${escapeXml(spec.cta)}</text>`;
    const splitPhoto=split&&spec.assetDataUri?`<image href="${escapeXml(spec.assetDataUri)}" x="0" y="0" width="${Math.round(preset.width*.47)}" height="${preset.height}" preserveAspectRatio="xMidYMid slice"/>`:'';
    const slide=spec.totalSlides&&spec.totalSlides>1?`<text x="${preset.width-safe}" y="${preset.height-safe}" text-anchor="end" font-family="${escapeXml(spec.fontFamily)}" font-size="22" fill="${fg}" opacity=".75">${(spec.slideIndex??0)+1}/${spec.totalSlides}</text>`:'';
    const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${preset.width}" height="${preset.height}" viewBox="0 0 ${preset.width} ${preset.height}"><rect width="100%" height="100%" fill="${bg}"/>${imageMode?photo:''}${imageMode?overlay:''}${splitPhoto}<rect x="${split?cardX-safe/2:0}" y="0" width="${split?cardW+safe:0}" height="${preset.height}" fill="${split?bg:'none'}"/>${headlineSvg}${bodySvg}${spec.cta?cta:''}${logo}${slide}</svg>`;
    const fingerprint=visualFingerprint({templateKey:spec.templateKey,visualType:spec.visualType,assetId:null,headline:spec.headline,background:bg,ctaPosition:'bottom_content'});
    return{status:issues.some((issue)=>issue.severity==='blocker'||issue.severity==='error')?'qa_failed':'ready',svg,width:preset.width,height:preset.height,issues,fingerprint};
  }
}
...
export class MockImageGenerationProvider implements ImageGenerationProvider {
  ...
  private async make(input:ImageGenerationRequest,mode:string):Promise<ImageGenerationResult>{
    const preset=formatPresets[input.format];
    const seed=hashNumber(`${input.tenantId}|${input.prompt}|${mode}`);
    const first=`#${((seed&0xffffff)|0x303030).toString(16).slice(-6)}`;
    const second=`#${(((seed>>>3)&0xffffff)|0x505050).toString(16).slice(-6)}`;
    const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${preset.width}" height="${preset.height}"><defs><linearGradient id="g"><stop stop-color="${first}"/><stop offset="1" stop-color="${second}"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/><circle cx="75%" cy="25%" r="180" fill="#fff" opacity=".16"/><path d="M0 ${preset.height*.72} Q ${preset.width*.45} ${preset.height*.45} ${preset.width} ${preset.height*.68} V ${preset.height} H0Z" fill="#fff" opacity=".12"/></svg>`;
    return{provider:'mock',assetType:'generated_visual',mimeType:'image/svg+xml',dataUri:`data:image/svg+xml;base64,${toBase64(svg)}`,promptHash:String(seed)};
  }
}
...
const toBase64=(value:string)=>{
  ...
  for(let index=0;index<bytes.length;index+=3){
    const first=bytes[index]??0,second=bytes[index+1]??0,third=bytes[index+2]??0;
    const triple=(first<<16)|(second<<8)|third;
    out+=alphabet.charAt((triple>>18)&63)+alphabet.charAt((triple>>12)&63)+(index+1<bytes.length?alphabet.charAt((triple>>6)&63):'=')+(index+2<bytes.length?alphabet.charAt(triple&63):'=');
  }
  return out;
};

export class ImagePromptBuilder {
  build(input:{brandName:string;visualBrief:VisualBrief;content:string;platform:VisualPlatform;style:string;forbiddenElements:string[];assetReferences:string[];photographicDirection:string}):{prompt:string;forbiddenElements:string[];referenceAssetIds:string[]}{
    const forbidden=unique(['invented logos','unverified products','fake customer testimonials','text embedded in photography',...input.forbiddenElements]);
    return{prompt:`Brand: ${input.brandName}. Objective: ${input.visualBrief.objective}. Subject: ${input.content}. Platform: ${input.platform}. Style: ${input.style}. Direction: ${input.photographicDirection}. No invented brand marks or factual claims.`,forbiddenElements:forbidden,referenceAssetIds:[...input.assetReferences]};
  }
}
...
export class SelectiveQaRepairEngine {
  repair(input:{component:RepairComponent;variant:RepairableVariant;issueCode:string;variantSeed:number}):{variant:RepairableVariant;repairAction:string}{
    const next=structuredClone(input.variant);
    let action='';
    if(input.component==='hook'){next.hook=shorten(next.hook,64)||'Un punto concreto da conoscere';action='rewrite_hook_only'}
    else if(input.component==='caption'){next.caption=shorten(next.caption,420);action='shorten_caption_only'}
    else if(input.component==='hashtags'){next.hashtags=unique(next.hashtags.map((tag)=>tag.startsWith('#')?tag:`#${tag.replace(/\s+/g,'')}`)).slice(0,8);action='replace_hashtags_only'}
    else if(input.component==='cta'){next.cta=shorten(next.cta,34)||'Scopri di più';action='rewrite_cta_only'}
    else{const keys=Object.keys(templateCatalog) as TemplateKey[];const current=keys.indexOf(next.visual.templateKey);next.visual.templateKey=keys[(Math.max(0,current)+1+input.variantSeed%3)%keys.length]!;next.visual.headline=shorten(next.visual.headline,68);next.visual.supportingText=shorten(next.visual.supportingText,150);action=input.issueCode==='TEMPLATE_REPETITION'?'rotate_template_only':'repair_visual_only'}
    return{variant:next,repairAction:action};
  }
}
const shorten=(value:string,max:number)=>value.trim().length<=max?value.trim():`${value.trim().slice(0,Math.max(1,max-1)).replace(/[\s,.;:!-]+$/,'')}…`;

export const buildCarouselSlides=(input:{type:NonNullable<VisualBrief['carouselType']>;headline:string;supportingText:string;cta:string}):Array<{slideIndex:number;headline:string;body:string;layout:string;visualType:VisualType}>=>{
  const count=input.type==='before_after'?4:5;
  return Array.from({length:count},(_,index)=>({slideIndex:index,headline:index===0?input.headline:index===count-1?'Prossimo passo':`${index}. ${carouselLabel(input.type,index)}`,body:index===count-1?input.cta:shorten(input.supportingText,130),layout:index===0?'cover':index===count-1?'cta':'content',visualType:'carousel'}));
};
