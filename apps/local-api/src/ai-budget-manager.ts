...
const usdMicros=(usd:number)=>Math.max(0,Math.ceil(usd*1_000_000));
const positive=(value:unknown)=>typeof value==='number'&&Number.isFinite(value)&&value>=0;
const asTextPrice=(value:unknown):TextPrice|null=>{
  if(!value||typeof value!=='object'||Array.isArray(value))return null;
  const row=value as Record<string,unknown>;
  if(!positive(row.inputUsdPer1M)||!positive(row.outputUsdPer1M))return null;
  return{inputUsdPer1M:Number(row.inputUsdPer1M),outputUsdPer1M:Number(row.outputUsdPer1M),...(positive(row.cachedInputUsdPer1M)?{cachedInputUsdPer1M:Number(row.cachedInputUsdPer1M)}:{})};
};
...
const parseCatalog=():PricingCatalog=>{
  ...
  return{version:obj.version.trim(),models:obj.models as Record<string,TextPrice|ImagePrice>};
};
const estimateTokens=(value:unknown)=>Math.max(1,Math.ceil(JSON.stringify(value??'').length/3));
...
const economyTasks=new Set(['classification','qa','dedupe','hashtags','rewrite','summary','metadata']);
const premiumEligibleTasks=new Set(['brand_refinement','strategy','post_generation']);
