...
export class OpenAIProductionClient {
  ...
  async json<T>(instruction:string,input:unknown):Promise<{value:T;model:string;responseId:string|null;usage:Record<string,unknown>}> {
    this.assertConfigured();
    const prompt=[
      instruction,
      'Return ONLY valid JSON. Do not use markdown fences. Do not add facts that are not supported by the supplied data.',
      'INPUT:',
      JSON.stringify(input),
    ].join('\n\n');
    const body=await this.post<ResponsesBody>('/responses',{model:this.textModel,input:prompt});
    ...
    try{return{value:JSON.parse(stripJsonFence(text)) as T,model:body.model??this.textModel,responseId:body.id??null,usage:body.usage??{}};}
    catch{throw new Error('OPENAI_INVALID_JSON');}
  }

  async text(instruction:string,input:unknown):Promise<{text:string;model:string;responseId:string|null;usage:Record<string,unknown>}> {
    this.assertConfigured();
    ...
    const text=(body.output??[]).flatMap((item)=>item.content??[]).filter((item)=>item.type==='output_text'||item.type==='text'||Boolean(item.text)).map((item)=>item.text??'').join('').trim();
    if(!text)throw new Error('OPENAI_EMPTY_RESPONSE');
    return{text,model:body.model??this.textModel,responseId:body.id??null,usage:body.usage??{}};
  }

  async image(prompt:string,size:'1024x1024'|'1024x1536'|'1536x1024'='1024x1536'):Promise<{bytes:Buffer;mimeType:'image/png';model:'gpt-image-2';revisedPrompt:string|null;usage:Record<string,unknown>}> {
    this.assertConfigured();
