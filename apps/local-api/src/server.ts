...
const route = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (url.pathname === '/health') { send(res, 200, { ok: true, mode: 'local-e2e', publishing: 'mock-only', visual: 'deterministic-local' }, cors); return; }
...
const server=createServer((req,res)=>{route(req,res).catch((error:unknown)=>{const message=error instanceof Error?error.message:String(error);const status=/auth_required|tenant_access_denied|platform_admin_required/.test(message)?403:/not_found|row_not_found|asset_not_found/.test(message)?404:/file_too_large|mime_not_supported/.test(message)?413:400;send(res,status,{error:message,local:true},corsHeaders(req))})});
