...
const corsHeaders = (req: IncomingMessage): Record<string,string> => { const origin = req.headers.origin; const allowed = new Set(['http://127.0.0.1:5173','http://localhost:5173','http://127.0.0.1:3000','http://localhost:3000']); return origin && allowed.has(origin) ? { 'access-control-allow-origin': origin, 'access-control-allow-headers': 'content-type, authorization, x-provider-signature, x-provider-timestamp, x-event-id, x-event-type, x-tenant-id, x-account-id', 'access-control-allow-methods': 'GET,POST,PATCH,DELETE,OPTIONS', 'vary': 'Origin' } : {}; };
...
const route = async (req: IncomingMessage, res: ServerResponse) => {
  const url = new URL(req.url ?? '/', `http://${req.headers.host ?? `${host}:${port}`}`); const parts = pathParts(url.pathname); const method = req.method ?? 'GET'; const cors = corsHeaders(req); if (method === 'OPTIONS') { res.writeHead(204, cors); res.end(); return; }
  ...
  if (parts[0] === 'fixture-site') { sendHtml(res, 200, fixtureSite(parts[1] ?? 'local-business', parts[2] ?? 'home')); return; }
  if (url.pathname === '/health') { send(res, 200, { ok: true, mode: 'local-e2e', publishing: 'mock-only', visual: 'deterministic-local', providers:'fixture-only' }, cors); return; }
