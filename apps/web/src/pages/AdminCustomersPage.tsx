...
export function AdminCustomersPage(){
  ...
  const load=async()=>{try{const next=await local.api<AdminSnapshot>('/admin/customers');setData(next);if(!tenantId&&next.tenants[0])setTenantId(String(next.tenants[0].id));if(!planCode&&next.plans[0])setPlanCode(String(next.plans[0].code));setMessage(null);}catch(e){setMessage(e instanceof Error?e.message:String(e));}};
  ...
  return <>
    <PageHeader eyebrow="Platform admin" title="Clienti, piani e operazioni" description="Il prodotto ha un unico stato operativo. Qui si gestiscono accessi, limiti e diagnostica; non esistono modalità demo/reale." action={<div className="card-actions"><button className="button secondary" onClick={()=>void claim()}>Claim admin locale</button><button data-testid="load-admin" className="button" onClick={()=>void load()}>Aggiorna</button></div>}/>
    {message&&<Card><p role="status">{message}</p></Card>}
    {!data?<Card><EmptyState title="Carica la console" body="Nessuna modifica viene eseguita senza controllo RBAC server-side."/></Card>:<>
