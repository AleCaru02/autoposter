...
const neonAuthUrl=!e2eFixtures?((import.meta.env.VITE_NEON_AUTH_URL as string|undefined)?.trim().replace(/\/$/,'')??''):'';
const neonDataApiUrl=!e2eFixtures?((import.meta.env.VITE_NEON_DATA_API_URL as string|undefined)?.trim().replace(/\/$/,'')??''):'';
const neonEnabled=Boolean(neonAuthUrl&&neonDataApiUrl);
const neonClient=neonEnabled?createClient({auth:{url:neonAuthUrl},dataApi:{url:neonDataApiUrl}}):null;
...
const freshNeonToken=async():Promise<string|null>=>{
  ...
  const getter=neonClient.auth.getJWTToken;if(typeof getter!=='function')throw new Error('Neon Auth JWT non disponibile');
  const value=await getter();return typeof value==='string'&&value?value:null;
};

export function LocalE2EProvider({children}:PropsWithChildren){
  const enabled=Boolean(baseUrl);const[health,setHealth]=useState<ProductHealth|null>(null);const[token,setToken]=useState<string|null>(()=>enabled?readStored(TOKEN_KEY,LEGACY_TOKEN_KEY):null);const[tenantId,setTenantId]=useState<string|null>(()=>enabled?readStored(TENANT_KEY,LEGACY_TENANT_KEY):null);const[tenants,setTenants]=useState<TenantSummary[]>([]);const[workspace,setWorkspace]=useState<LocalWorkspace|null>(null);const[loading,setLoading]=useState(false);const[error,setError]=useState<string|null>(null);const refreshSequence=useRef(0);
  const storeSession=useCallback((accessToken:string)=>{localStorage.setItem(TOKEN_KEY,accessToken);localStorage.removeItem(LEGACY_TOKEN_KEY);setToken(accessToken);},[]);
  ...
  const resolvedToken=useCallback(async()=>{if(neonClient){const next=await freshNeonToken();if(next&&next!==token)storeSession(next);return next;}return token;},[token,storeSession]);
  ...
  useEffect(()=>{if(!neonClient)return;void freshNeonToken().then((next)=>{if(next)storeSession(next);else clearSession();}).catch(()=>clearSession());},[storeSession,clearSession]);
  ...
  const register=async(input:{name:string;email:string;password:string})=>{
    setLoading(true);setError(null);try{
      if(neonClient){await neonClient.auth.signUp.email({email:input.email,password:input.password,name:input.name});const accessToken=await freshNeonToken();if(!accessToken)throw new Error('Account creato. Completa l’eventuale verifica email e poi accedi.');storeSession(accessToken);return;}
  ...
  const login=async(input:{email:string;password:string})=>{
    setLoading(true);setError(null);try{
      if(neonClient){await neonClient.auth.signIn.email({email:input.email,password:input.password});const accessToken=await freshNeonToken();if(!accessToken)throw new Error('Accesso Neon completato ma sessione JWT non disponibile.');storeSession(accessToken);return;}
  ...
  const logout=()=>{refreshSequence.current+=1;if(neonClient)void neonClient.auth.signOut().catch(()=>undefined);clearSession();};
