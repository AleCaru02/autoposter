...
interface NeonAuthEnvelope { session?: NeonSessionPayload | null; user?: Record<string,unknown> | null; data?: { session?: NeonSessionPayload | null; user?: Record<string,unknown> | null } | null; error?: { message?: string } | string | null; message?: string; }
...
const neonEnabled=Boolean(neonAuthUrl);
...
const neonAuthRequest=async(path:string,init:RequestInit={}):Promise<NeonAuthEnvelope>=>{
  if(!neonAuthUrl)throw new Error('Neon Auth non configurato');
  const headers=new Headers(init.headers);if(init.body&&!headers.has('content-type'))headers.set('content-type','application/json');
  const response=await fetch(`${neonAuthUrl}/auth/${path}`,{...init,headers,credentials:'include'});
