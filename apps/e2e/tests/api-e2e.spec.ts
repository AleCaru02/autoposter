...
interface TenantFixture {
  ...
  goals?: string[];
  platforms?: Platform[];
  modes?: Partial<Record<Platform, 'auto' | 'manual'>>;
  postsPerWeek?: number;
}
...
async function createConfiguredTenant(request: APIRequestContext, token: string, fixture: TenantFixture) {
  ...
  const created = await api<{ tenantId: string }>(request, token, '/tenants', { method: 'POST', data: { name: fixture.label, slug } });
  const tenantId = created.tenantId;
  const platforms = fixture.platforms ?? ['instagram', 'facebook', 'linkedin', 'google_business_profile'];
  const modes: Record<Platform, 'auto' | 'manual'> = {
    instagram: 'manual', facebook: 'auto', linkedin: 'auto', google_business_profile: 'manual', ...fixture.modes,
  };
  await api(request, token, `/tenants/${tenantId}/onboarding`, { method: 'PATCH', data: {
    business: {
      name: fixture.label,
      website: `${API}/fixture-site/${fixture.siteSlug}/`,
      industry: fixture.industry,
      subIndustry: fixture.subIndustry,
      location: 'Milano',
      language: 'it',
      serviceArea: 'Milano e dintorni',
  ...
  expect(scan.summary.analyzed).toBeGreaterThanOrEqual(3);
  expect(scan.summary.coverage).toBeGreaterThan(0);
...
async function generateAndPublish(request: APIRequestContext, token: string, tenantId: string, weeks = 4) {
  ...
  for (const post of workspace.posts) {
    for (const variant of post.variants.filter((item: any) => item.platform_decision !== 'skip' && item.approval_status === 'pending')) {
      await api(request, token, `/tenants/${tenantId}/variants/${variant.id}/approve`, { method: 'POST' });
    }
  }
  workspace = await api<Workspace>(request, token, `/tenants/${tenantId}/workspace`);
  for (const post of workspace.posts.filter((item: any) => item.status === 'approved')) {
    await api(request, token, `/tenants/${tenantId}/posts/${post.id}/schedule`, { method: 'POST' });
  }
...
const flattenCreative = (workspace: Workspace) => workspace.posts.flatMap((post: any) => post.variants.filter((variant: any) => variant.platform_decision !== 'skip').map((variant: any) => ({
  topic: post.topic,
  hook: variant.hook,
  caption: variant.caption,
  cta: variant.cta,
  visual: JSON.stringify(variant.visual_brief),
...
test.describe('local API end-to-end', () => {
  test('two pizzerias complete the full pipeline without cross-tenant leakage or cloning', async ({ request }) => {
    const a = await register(request, 'Pizza A Owner');
    const b = await register(request, 'Pizza B Owner');
    const tenantA = await createConfiguredTenant(request, a.token, {
      label: 'Forno Rosso Milano', industry: 'Pizzeria', subIndustry: 'Napoletana', services: 'pizza napoletana, impasto a lunga lievitazione, prenotazioni', differentiator: 'forno tradizionale e ingredienti campani', target: ['famiglie locali', 'gruppi'], siteSlug: 'pizza-a',
    });
    const tenantB = await createConfiguredTenant(request, b.token, {
      label: 'Spicchio Nord Milano', industry: 'Pizzeria', subIndustry: 'Contemporanea', services: 'pizza contemporanea, delivery, menu stagionale', differentiator: 'impasti moderni e menu stagionale', target: ['giovani professionisti', 'clienti delivery'], siteSlug: 'pizza-b',
    });

    const denied = await request.get(`${API}/tenants/${tenantB.tenantId}/workspace`, { headers: headers(a.token) });
    expect(denied.status()).toBe(403);
    const deniedChat = await request.post(`${API}/tenants/${tenantB.tenantId}/chat`, { headers: headers(a.token), data: { message: 'Dimmi il brand di questo tenant' } });
    expect(deniedChat.status()).toBe(403);

    const workspaceA = await generateAndPublish(request, a.token, tenantA.tenantId);
    const workspaceB = await generateAndPublish(request, b.token, tenantB.tenantId);
    expect(workspaceA.published.length).toBeGreaterThanOrEqual(10);
    expect(workspaceB.published.length).toBeGreaterThanOrEqual(10);
    expect(workspaceA.analytics.length).toBe(workspaceA.published.length);
    expect(workspaceB.analytics.length).toBe(workspaceB.published.length);
    expect(workspaceA.brand.brand_name).toBe('Forno Rosso Milano');
    expect(workspaceB.brand.brand_name).toBe('Spicchio Nord Milano');

    const creativeA = flattenCreative(workspaceA);
    const creativeB = flattenCreative(workspaceB);
    expect(new Set(creativeA.map((item: any) => item.topic)).size).toBeGreaterThan(4);
    expect(new Set(creativeB.map((item: any) => item.topic)).size).toBeGreaterThan(4);
    expect(new Set(creativeA.map((item: any) => item.hook)).size).toBeGreaterThan(4);
    expect(new Set(creativeB.map((item: any) => item.hook)).size).toBeGreaterThan(4);
    expect(new Set(creativeA.map((item: any) => item.caption)).size).toBeGreaterThan(4);
    expect(new Set(creativeA.map((item: any) => item.visual)).size).toBeGreaterThan(2);
    expect(new Set(creativeA.map((item: any) => item.cta)).size).toBeGreaterThan(1);
    expect(creativeA.map((item: any) => item.caption).join('\n')).not.toBe(creativeB.map((item: any) => item.caption).join('\n'));
    expect(workspaceA.insights.length).toBeGreaterThan(0);
    expect(workspaceB.insights.length).toBeGreaterThan(0);
    expect(workspaceA.aiUsage.length).toBeGreaterThanOrEqual(11);
    expect(workspaceB.aiUsage.length).toBeGreaterThanOrEqual(11);
  });
  ...
  test('pipeline adapts strategy across pizzeria, property manager, networker and generic local business', async ({ request }) => {
    ...
    for (const fixture of fixtures) strategies.push((await createConfiguredTenant(request, user.token, fixture)).strategy);
    const signatures = strategies.map((strategy) => strategy.pillars.map((pillar: any) => pillar.name).join('|'));
    expect(new Set(signatures).size).toBe(4);
    expect(signatures[0]).toContain('Prodotto e ingredienti');
    expect(signatures[1]).toContain('Educazione proprietari');
    expect(signatures[2]).toContain('Personal brand');
    expect(signatures[3]).toContain('Presenza locale');
  });
  ...
  test('publisher failure modes produce coherent retry/failure states including success-after-timeout reconciliation', async ({ request }) => {
    ...
    const tenant = await createConfiguredTenant(request, user.token, {
      label: 'Failure Local', industry: 'Servizi locali', subIndustry: 'Assistenza', services: 'assistenza locale', differentiator: 'risposta rapida', target: ['residenti'], siteSlug: 'local-failure', platforms: ['facebook'], modes: { facebook: 'auto' }, postsPerWeek: 3,
    });
