...
test('platform admin manages a first client without Stripe and REAL mode blocks fixture generation',async({request})=>{
  ...
  const snapshot=await request.get(`${API}/admin/customers`,{headers:auth});expect(snapshot.ok()).toBeTruthy();const admin=await snapshot.json();expect(admin.tenants.some((tenant:any)=>tenant.id===tenantId)).toBe(true);expect(admin.plans.some((plan:any)=>plan.code==='local-dev')).toBe(true);
  ...
  const real=await request.patch(`${API}/admin/tenants/${tenantId}/data-mode`,{headers:auth,data:{dataMode:'REAL'}});expect(real.ok()).toBeTruthy();
  const blocked=await request.post(`${API}/tenants/${tenantId}/strategy`,{headers:auth});expect(blocked.status()).toBe(403);expect((await blocked.json()).error).toContain('REAL_PROVIDER_NOT_CONFIGURED');
  const workspace=await request.get(`${API}/tenants/${tenantId}/workspace`,{headers:auth});expect(workspace.ok()).toBeTruthy();const realWorkspace=await workspace.json();expect(realWorkspace.connections).toHaveLength(0);expect(realWorkspace.analytics).toHaveLength(0);expect(realWorkspace.published).toHaveLength(0);

  const demo=await request.patch(`${API}/admin/tenants/${tenantId}/data-mode`,{headers:auth,data:{dataMode:'DEMO'}});expect(demo.ok()).toBeTruthy();
