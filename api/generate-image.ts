...
type AssetRow = { id: string; source?: string; kind?: string; name?: string; storage_url?: string; mime_type?: string | null; tags?: unknown; metadata?: unknown; created_at?: string };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const requestedAspectRatio = requestedFormat === "STORY" ? "9:16" : "1:1";
    const assetCandidates = await readRows<ReusableAssetCandidate>(
    ...
    if (savedVariant) {
      ...
      if (savedVariant.image_asset_id && savedVariant.image_asset_id !== asset.id) {
        const references = await readRows<{ id: string }>(`content_variants?profile_id=eq.${encodeURIComponent(profileId)}&image_asset_id=eq.${encodeURIComponent(savedVariant.image_asset_id)}&select=id&limit=1`, token);
