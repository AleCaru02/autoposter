...
function defaultTextBudget(env: AutopilotEnv) { return Math.min(Math.max(numberOr(env.OPENAI_TEXT_MONTHLY_BUDGET_USD, DEFAULT_TEXT_BUDGET_USD), 0.1), 100); }
function defaultImageLimit(env: AutopilotEnv) { return Math.min(Math.max(Math.floor(numberOr(env.OPENAI_IMAGE_MONTHLY_LIMIT, DEFAULT_IMAGE_LIMIT)), 1), 500); }
...
function guardedEnv(env: AutopilotEnv, policy: ReturnType<typeof profileAiEconomicsPolicy>, usage: UsageSnapshot, approvalMode: string | null) {
  const remainingUsd=Math.max(policy.monthlyAiBudgetUsd-usage.profileTotalUsd,0);const remainingImages=Math.max(policy.monthlyImageLimit-usage.profileImages,0);const allowAutopilotImage=approvalMode==="AUTOMATIC"||policy.generateImagesAfterApproval===false;
  return{...env,OPENAI_TEXT_MONTHLY_BUDGET_USD:String(usage.globalTextUsd+remainingUsd),OPENAI_IMAGE_MONTHLY_LIMIT:String(usage.globalImages+(allowAutopilotImage?remainingImages:0))};
}
...
export async function runContentAutopilotSerialized(env: AutopilotEnv, options: RunOptions = {}) {
  ...
  try{
    ...
    for(const profileId of ids){
      if(remainingRun<=0)break;let strategy=await platformStrategy(client,profileId);let strategyObject=strategy&&typeof strategy==="object"&&!Array.isArray(strategy)?strategy as Record<string,unknown>:{};const policy=profileAiEconomicsPolicy(strategy,{monthlyAiBudgetUsd:defaultTextBudget(env),monthlyImageLimit:defaultImageLimit(env)});let usage=await usageSnapshot(client,profileId);
      if(usage.profileTotalUsd>=policy.monthlyAiBudgetUsd){aggregate.profilesChecked+=1;aggregate.skipped+=1;aggregate.errors.push(`${profileId}:AUTOPILOT_AI_LIMIT_REACHED:MONTHLY_AI_BUDGET`);continue;}
      ...
      if(reserve>0&&usage.profileTotalUsd+reserve>policy.monthlyAiBudgetUsd){aggregate.profilesChecked+=1;aggregate.skipped+=1;aggregate.errors.push(`${profileId}:AUTOPILOT_AI_LIMIT_REACHED:PLANNER_REFRESH_RESERVE`);continue;}
      if(refresh.refreshStrategy||refresh.refreshPlan){await ensureOpenAIStrategyPlannerFresh(env,profileId,{strategyRefreshDays:policy.strategyRefreshDays,planRefreshDays:policy.planRefreshDays});strategy=await platformStrategy(client,profileId);strategyObject=strategy&&typeof strategy==="object"&&!Array.isArray(strategy)?strategy as Record<string,unknown>:{};usage=await usageSnapshot(client,profileId);if(usage.profileTotalUsd>=policy.monthlyAiBudgetUsd){aggregate.profilesChecked+=1;aggregate.skipped+=1;aggregate.errors.push(`${profileId}:AUTOPILOT_AI_LIMIT_REACHED:MONTHLY_AI_BUDGET_AFTER_PLANNING`);continue;}}
      ...
      const scopedEnv=guardedEnv(env,policy,usage,typeof strategyObject.approvalMode==="string"?strategyObject.approvalMode:null);const result=await runContentAutopilot(scopedEnv,{profileId,maxGenerations:profileGenerationCap});addResult(aggregate,result);remainingRun-=result.generated;
    }
    return aggregate;
