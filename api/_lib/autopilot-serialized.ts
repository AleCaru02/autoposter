...
export async function runContentAutopilotSerialized(env: AutopilotEnv, options: RunOptions = {}) {
  ...
  try{
    ...
    for(const profileId of ids){
      if(remainingRun<=0)break;let strategy=await platformStrategy(client,profileId);let strategyObject=strategy&&typeof strategy==="object"&&!Array.isArray(strategy)?strategy as Record<string,unknown>:{};const policy=profileAiEconomicsPolicy(strategy);let usage=await usageSnapshot(client,profileId);
      ...
      const approvalMode=typeof strategyObject.approvalMode==="string"?strategyObject.approvalMode:null;const allowImageGeneration=approvalMode==="AUTOMATIC"||policy.generateImagesAfterApproval===false;const result=await runContentAutopilot(env,{profileId,maxGenerations:profileGenerationCap,allowImageGeneration});addResult(aggregate,result);remainingRun-=result.generated;
    }
    return aggregate;
