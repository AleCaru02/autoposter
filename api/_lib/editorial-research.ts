...
export function buildEditorialResearchPolicy(modeValue: unknown): EditorialResearchPolicy {
  ...
  if (mode === "WEBSITE_ONLY") {
    return { mode, useWebSearch: false, freshnessDays: null, instruction: "Usa il sito come unica fonte fattuale. Non introdurre informazioni esterne." };
  }
  if (mode === "NEWS") {
    return { mode, useWebSearch: true, freshnessDays: 14, instruction: "Cerca aggiornamenti e notizie recenti realmente pertinenti al settore. Preferisci fonti primarie, istituzionali o editoriali affidabili; evita trend generici senza utilità per il pubblico del brand." };
  }
  if (mode === "TIPS") {
    return { mode, useWebSearch: true, freshnessDays: null, instruction: "Cerca informazioni affidabili utili per creare consigli pratici, spiegazioni, errori da evitare, checklist e contenuti educativi pertinenti al settore." };
  }
  if (mode === "EVERGREEN") {
    return { mode, useWebSearch: true, freshnessDays: null, instruction: "Cerca conoscenze affidabili e durature del settore. Evita di forzare notizie o trend; privilegia contenuti evergreen utili anche tra mesi." };
  }
...
export function buildSectorResearchInstruction(input: { industry: string | null; description: string | null; businessModel: string | null; target: string | null; mode?: unknown }) {
