...
async function createPlannedContent(input:{sql:Sql;env:Required<Pick<AutopilotEnv,"OPENAI_API_KEY">>&AutopilotEnv;profile:ProfileRow;strategy:StrategyRow|undefined;provider:SocialProvider;scheduledAt:string;approvalMode:ApprovalMode;budget:{textSpent:number;textLimit:number;imagesUsed:number;imageLimit:number}}){
  ...
  const topics=await recentTopics(sql,profile.id);const count=await recentVariantCount(sql,profile.id,provider);const planItem=selectPlanItem(strategy?.platform_strategy,provider,scheduledAt);const format=chooseAutopilotPublishFormat(provider,count,planItem?.format);const effectivePlanItem=planItem?{...planItem,format}:null;const objective=planItem?.objective||strings(strategy?.objectives)[0]||context.goals[0]||null;
