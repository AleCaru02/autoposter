...
export function chooseAutopilotPublishFormat(provider:SocialProvider,recentCount:number,requested?:SocialFormat|null):SocialFormat{const supported=AUTOPILOT_PUBLISH_FORMATS[provider];if(requested&&supported.includes(requested))return requested;return supported[recentCount%supported.length]??"POST";}
...
async function createPlannedContent(input:{sql:Sql;env:Required<Pick<AutopilotEnv,"OPENAI_API_KEY">>&AutopilotEnv;profile:ProfileRow;strategy:StrategyRow|undefined;provider:SocialProvider;scheduledAt:string;approvalMode:ApprovalMode;budget:{textSpent:number;textLimit:number;imagesUsed:number;imageLimit:number}}){
  ...
  const topics=await recentTopics(sql,profile.id);const count=await recentVariantCount(sql,profile.id,provider);const planItem=selectPlanItem(strategy?.platform_strategy,provider,scheduledAt);const format=chooseAutopilotPublishFormat(provider,count,planItem?.format);const effectivePlanItem=planItem?{...planItem,contentType:chooseAutopilotContentType(format),format}:null;const objective=planItem?.objective||strings(strategy?.objectives)[0]||context.goals[0]||null;
  ...
  const topicRequest=effectivePlanItem?buildPlanDrivenTopicRequest(effectivePlanItem,topics):[pillar.instruction||"Scegli autonomamente un nuovo tema editoriale specifico e utile per questa attività.","Per i fatti specifici dell'attività usa solo sito e brand; per conoscenze di settore, consigli e aggiornamenti segui il filtro editoriale e usa ricerca esterna verificata quando consentita.",`Il contenuto è destinato a ${provider} nel formato ${format}.`,topics.length?`Evita di ripetere questi temi recenti: ${topics.join(" | ")}.`:"Evita temi generici e ripetitivi."].join(" ");
  ...
  try{
  const upper=estimateTextRequestUpperBoundUsd({topic:topicRequest,objective,providers:[provider],formats:[format],brand:context,researchMode});const qaReserve=approvalMode==="AUTOMATIC"?AUTO_QA_RESERVE_USD:0;if(budget.textSpent>=budget.textLimit||budget.textSpent+upper+qaReserve>budget.textLimit){await meter.release(logicalEventId,"AI_BUDGET_EXCEEDED");throw new Error("AUTOPILOT_TEXT_BUDGET_REACHED");}
