...
async function createPlannedContent(input:{sql:Sql;env:Required<Pick<AutopilotEnv,"OPENAI_API_KEY">>&AutopilotEnv;profile:ProfileRow;strategy:StrategyRow|undefined;provider:SocialProvider;scheduledAt:string;approvalMode:ApprovalMode}){
  const{sql,env,profile,strategy,provider,scheduledAt,approvalMode}=input;const loaded=await loadBrandContext(sql,profile);const context=loaded.context;if(!context.confirmedWebsiteContent.length)throw new Error("AUTOPILOT_WEBSITE_CONTEXT_MISSING");
