...
export type ImageTechnicalEvent = {
  operation: string;
  model: string;
...
export type MeteredImageResult = { technicalEvents: ImageTechnicalEvent[] };
...
export type ImageGenerationSource = "MANUAL" | "AUTOPILOT";
type CachedResult = { response: unknown; assetId?: string | null; variantId?: string | null };
...
export async function deriveImageGenerationOperationKey(input: {
  profileId: string;
  ...
  return `ai-image:v1:${await sha256(`${input.profileId}|${input.source}|${input.operationIdentity}|${fingerprint}`)}`;
}

export class ImageGenerationMetering {
  ...
  async reserve(input: {
    ...
    const operationKey = await deriveImageGenerationOperationKey(input);
    const reserved = await this.usage.reserveUsage({
      profileId: input.profileId,
      capabilityKey: AI_IMAGE_GENERATE_CAPABILITY,
      quantity: 1,
      idempotencyKey: operationKey,
      source: `AI_IMAGE_${input.source}`,
  ...
  async persistTechnicalEvents(profileId: string, eventId: string, events: ImageTechnicalEvent[]) {
    ...
    const payload = events.map((event) => ({
      ...
      metadata: { ...event.metadata, logical_usage_event_id: eventId, logical_capability: AI_IMAGE_GENERATE_CAPABILITY },
    }));
    await this.usage.mergeUsageEventMetadata(eventId, { technical_usage_outbox: payload, technical_usage_state: "PENDING" });
    let lastError: unknown = null;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        await this.sql`
          insert into public.ai_usage_events (profile_id,operation,model,input_tokens,output_tokens,cost_usd,metadata)
          select ${profileId}::uuid, item->>'operation', item->>'model', nullif(item->>'input_tokens','')::integer,
                 nullif(item->>'output_tokens','')::integer, nullif(item->>'cost_usd','')::numeric,
  ...
  async storeResult(eventId: string, cached: CachedResult) {
    await this.usage.mergeUsageEventMetadata(eventId, { execution_state: "OUTPUT_PERSISTED", cached_result: cached });
  }
...
export function technicalEventsFromImageResult(result: MeteredImageResult, metadata: Record<string, unknown> = {}) {
