-- Include variant-scoped image generations in their parent post cost rollup.
...
begin
  ...
  with eligible as (
    select tm.tenant_id from public.tenant_members tm where tm.user_id=v_user and tm.role='owner' and tm.status='active'
  ), base as (
    select ue.id,ue.tenant_id,t.name as tenant_name,
      coalesce(ue.post_id,pv.post_id) as post_id,
      ue.post_variant_id,ue.task,ue.model,ue.model_tier,ue.spend_kind,
      ...
      p.topic,coalesce(pv.format,p.format,'unknown') as format,
      coalesce(pv.content_mode,p.content_mode,'unknown') as content_mode,pv.platform
    ...
    left join public.posts p on p.id=coalesce(ue.post_id,pv.post_id) and p.tenant_id=ue.tenant_id
    where ue.created_at>=v_from and ue.created_at<v_to and (p_scope='portfolio' or ue.tenant_id=p_tenant_id)
  ), daily as (
    ...
    select coalesce(sum(r.estimated_usd_micros),0)::bigint cost from app_private.ai_spend_reservations r join eligible e on e.tenant_id=r.tenant_id
  ...
  select jsonb_build_object(
    ...
    'totalSpentMicros',coalesce((select sum(cost_micros) from base),0),'reservedMicros',(select cost from reserved),'calls',coalesce((select count(*) from base),0),
