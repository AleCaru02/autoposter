...
export function ProfilesPage() {
  ...
  async function archiveSelectedActivity() {
    if (!selectedProfile || archiving) return;
    ...
    try {
      ...
      const response = await fetch("/api/profile-archive", {
        method: "POST",
        headers: {
      ...
      const body = await response.json().catch(() => ({})) as { archived?: boolean; error?: string };
      if (!response.ok || body.archived !== true) throw new Error(body.error || "PROFILE_ARCHIVE_FAILED");
      await reload();
    } catch {
