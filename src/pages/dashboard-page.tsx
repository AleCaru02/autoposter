...
export function DashboardPage() {
  ...
  const databaseOk = health?.ready === true && health.database === "postgres_ready";
  const databaseLabel = databaseOk ? `Attivo${typeof health?.latencyMs === "number" ? ` · ${health.latencyMs} ms` : ""}` : healthError ? "Verifica non disponibile" : health ? health.database === "not_configured" ? "Non configurato" : health.database === "postgres_error" || health.database === "postgres_unhealthy" ? "Non disponibile" : "Verifica non riuscita" : "Verifica…";
