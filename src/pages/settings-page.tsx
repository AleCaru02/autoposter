...
export function SettingsPage() {
  ...
  return <div className="page-content">
    ...
    <section className="panel"><div className="panel-heading"><div><h2>Sicurezza</h2><p>Per cambiare password devi confermare prima quella attuale.</p></div><ShieldCheck size={19} /></div><form className="form-grid" onSubmit={changePassword}>
      <label className="full">Password attuale<div className="password-input-wrap"><input type={showCurrentPassword ? "text" : "password"} autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /><button type="button" className="password-eye" aria-label={showCurrentPassword ? "Nascondi password" : "Mostra password"} onClick={() => setShowCurrentPassword((value) => !value)}>{showCurrentPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>
      <label>Nuova password<div className="password-input-wrap"><input type={showNewPassword ? "text" : "password"} autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} /><button type="button" className="password-eye" aria-label={showNewPassword ? "Nascondi password" : "Mostra password"} onClick={() => setShowNewPassword((value) => !value)}>{showNewPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>
      ...
      {passwordDone && <p className="form-success full" role="status">{passwordDone}</p>}
