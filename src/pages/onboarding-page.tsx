...
import { Check, Globe2, LoaderCircle, Sparkles, WandSparkles } from "lucide-react";
...
type Stage = "FORM" | "CRAWL" | "ANALYZE" | "DONE";
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const scanResponse = await fetch("/api/website-scan", {
        ...
        body: JSON.stringify({ profileId: created.id, pageLimit: 500 }),
      });
      ...
      const analysisResponse = await fetch("/api/onboarding-analyze", {
        method: "POST",
        headers: { authorization: `Bearer ${token}`, "content-type": "application/json" },
        body: JSON.stringify({ profileId: created.id, visualHints: hints }),
      });
